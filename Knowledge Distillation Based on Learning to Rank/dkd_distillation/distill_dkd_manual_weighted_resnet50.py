...
#DKD loss with manually fixed weights (TCKD=4, NCKD=1)
...
def dkd_tckd_nckd(student_logits, teacher_logits, labels, T=4.0, eps=1e-12):
    ...
    p_s = F.softmax(s_logits_T, dim=1)   # (N, C)
    p_t = F.softmax(t_logits_T, dim=1)   # (N, C)

    labels = labels.view(-1, 1)          # (N, 1)

    #TCKD
    pt_s = p_s.gather(1, labels)        # (N, 1)
    pt_t = p_t.gather(1, labels)        # (N, 1)

    pnt_s = 1.0 - pt_s                  # (N, 1)
    pnt_t = 1.0 - pt_t                  # (N, 1)

    b_s = torch.cat([pt_s, pnt_s], dim=1)   # (N, 2)
    b_t = torch.cat([pt_t, pnt_t], dim=1)   # (N, 2)

    # KL
    tckd = torch.sum(
        b_t * (torch.log(b_t + eps) - torch.log(b_s + eps)),
        dim=1
    ).mean() * (T ** 2)
...
class ManualWeightedDKDLoss(nn.Module):
    """
    DKD with manually fixed weights for TCKD and NCKD.
    Here alpha=4 and beta=1, no learnable uncertainty parameters.
    """
    def __init__(self, temperature=4, alpha=4.0, beta=1.0):
        super().__init__()
        self.temperature = temperature
        self.alpha = alpha  # TCKD weight
        self.beta = beta    # NCKD weight
    ...
    def forward(self, student_logits, labels, teacher_logits):
        ...
        loss = self.alpha * L_tckd + self.beta * L_nckd
        return loss, L_tckd.detach(), L_nckd.detach()

criterion = ManualWeightedDKDLoss(temperature=temperature, alpha=4.0, beta=1.0).to(device)

#No learnable parameters in the loss; optimize the model only
...
def train_model(model, optimizer, scheduler, num_epochs=num_epochs, criterion=criterion):
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels, t_logits in distillation_loader:
            ...
            #Compute and backpropagate DKD with fixed weights
            loss, L_tckd_det, L_nckd_det = criterion(
        ...
        print(f'Fixed TCKD weight: 4.0000, NCKD weight: 1.0000')
...
torch.save(model.state_dict(), 'saved_models/manual_weight_DKD_T4_N1_final_model.pth')
