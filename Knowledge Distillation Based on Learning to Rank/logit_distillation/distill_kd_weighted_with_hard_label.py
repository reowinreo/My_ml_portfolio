...
num_epochs = 500
temperature = 4
lambda_ce = 2  # hard label weight is lambda
...
def train_model(model, optimizer, scheduler, num_epochs=num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        # Train phase with KD (weight 1) + λ*CE
        model.train()
        running_loss = 0.0
        running_corrects = 0
        ...
        for inputs, labels, t_logits in distillation_loader:
            ...
            loss_kd = distillation_loss(outputs, t_logits, temperature)
            loss = loss_kd + lambda_ce * loss_ce  # Teacher fixed at 1, hard label weighted by λ

            # Training accuracy still computed as "student vs teacher hard predictions" (unchanged)
        ...
        # Validation phase (still uses hard labels for evaluation, unchanged)
        model.eval()
...
# As requested "everything else unchanged", keep the original save filename
torch.save(model.state_dict(), 'saved_models/lambda2_teacher_plus_hard_label_T4_weight_distilled_final_model.pth')
