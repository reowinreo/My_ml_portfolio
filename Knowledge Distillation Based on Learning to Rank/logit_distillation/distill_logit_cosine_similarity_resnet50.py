...
# ========= Modified part: use cosine similarity =========
def distillation_loss(student_logits, teacher_logits, T):
    ...
    # Normalize
    s_norm = nn.functional.normalize(s, p=2, dim=1)
    t_norm = nn.functional.normalize(t, p=2, dim=1)
...
def train_model(model, optimizer, scheduler, num_epochs=num_epochs):
    ...
    class DistillationDataset(torch.utils.data.Dataset):
        ...
        def __getitem__(self, idx):
            img, _ = self.dataset[idx]  # Ignore hard labels
            return img, self.teacher_logits[idx]
    ...
    for epoch in range(num_epochs):
        ...
        # Train phase with distillation
        model.train()
        running_loss = 0.0
        running_corrects = 0

        for inputs, t_logits in distillation_loader:
            inputs = inputs.to(device)
            t_logits = t_logits.to(device)
            optimizer.zero_grad()

            outputs = model(inputs)
            loss = distillation_loss(outputs, t_logits, temperature)
            _, preds = torch.max(outputs, 1)
            _, t_preds = torch.max(t_logits, 1)  # For accuracy, use teacher's hard preds as reference
        ...
        # Validation phase (still uses hard labels for evaluation)
        model.eval()
...
torch.save(model.state_dict(), 'saved_models/cos_distilled_final_model.pth')
