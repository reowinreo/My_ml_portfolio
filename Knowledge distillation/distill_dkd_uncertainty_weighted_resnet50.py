...
class UncertaintyWeightedKDLoss(nn.Module):
    ...
    def forward(self, student_logits, labels, teacher_logits):
        ...
        return loss, L_tckd.detach(), L_nckd.detach(), sigma_t.detach(), sigma_n.detach()
...
def train_model(model, optimizer, scheduler, num_epochs=num_epochs, criterion=criterion):
    ...
    for epoch in range(num_epochs):
        ...
        try:
            eff_w_t = float((1.0 / (2.0 * (sigma_t_det**2))).cpu().numpy())
            eff_w_n = float((1.0 / (2.0 * (sigma_n_det**2))).cpu().numpy())
            print(f'train Loss: {epoch_loss:.4f} Acc: {epoch_acc:.4f}')
            print(f'Current TCKD weight: {eff_w_t:.4f}, NCKD weight: {eff_w_n:.4f}')
        except Exception:
            print(f'train Loss: {epoch_loss:.4f} Acc: {epoch_acc:.4f}')
...
torch.save(model.state_dict(), 'saved_models/auto_weight_update_DKD_only_final_model.pth')
