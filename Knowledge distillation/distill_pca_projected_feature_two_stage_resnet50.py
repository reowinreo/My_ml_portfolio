...
# EMA/LR scaling rules for Stage 1 (keep original logic)
...
# Extract penultimate layer (no-dropout version)
...
# ---------- Extract normalized 2048D teacher features ----------
...
print("Extracting teacher features for train set")
q_train_2048 = extract_teacher_feats(train_dataset, batch_size=batch_size)  # [Ntrain, 2048]
print("Extracting teacher features for val set")
...
pca.fit(q_train_2048)   # Fit on normalized train features

k = pca.components_.shape[0]     # get k adaptively
pca_mean = pca.mean_.astype(np.float32)          # [2048]
pca_P = pca.components_.astype(np.float32)       # [k, 2048]
print(f"PCA completed: k={k}")

# Use the same (mean, P) to project teacher train/val features to k-dim q
...
# Put mean and P on GPU to project student features during training
pca_mean_t = torch.from_numpy(pca_mean).to(device)          # [2048]
pca_P_t    = torch.from_numpy(pca_P).to(device)             # [k, 2048]

# ---------- Distillation dataset: return (image, q_k) ----------
...
# Stage 1: freeze fc and train backbone only
for p in student_wrap.fc.parameters():
    p.requires_grad = False

# Stage 1 optimizer + cosine annealing (warm restarts) — keep original logic
...
def project_student_to_k(student_feats_2048: torch.Tensor) -> torch.Tensor:
    """
    Student 2048D -> normalize -> project to k-dim using teacher mean/P
    inputs: [B, 2048]
    returns: [B, k]
    """
...
# --------------------
# Stage 1: PCA feature alignment (supervised by q_k), save by minimal val_MSE; EMA(val_MSE) only triggers LR decay
# --------------------
print("\n===== Stage 1 =====")
...
for epoch in range(num_epochs_stage1):
    ...
    # EMA(val_mse) -> only used to trigger LR÷5
    if ema is None:
        ema = val_mse
        prev_ema = ema
    else:
    ...
    if val_mse < best_val_mse:
        ...
        print(f"Saved best Stage 1 model (val_mse={val_mse:.6f})")

    # LR decay trigger
    if no_improve_epochs > stage1_patience_trigger:
        print(f"patience({no_improve_epochs}) > {stage1_patience_trigger}, divide LR by 5")
        scale_lr_with_scheduler(optim_stage1, sched_stage1, lr_scale_factor)
        no_improve_epochs = 0

    sched_stage1.step()

# Load best Stage 1 (as start point for Stage 2)
student_wrap.load_state_dict(best_stage1_state)

# --------------------
# Stage 2 (train fc only): LR=0.01, StepLR every 10 epochs ×0.5; no EMA
# --------------------
print("\n===== Stage 2 =====")
