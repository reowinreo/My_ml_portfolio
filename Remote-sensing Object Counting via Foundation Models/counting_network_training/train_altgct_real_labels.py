...
CONFIG = {
    ...
    "lambda_local": 0.5,          # λ
    "eta_reg": 0,               # η(regression L1Loss weight)
...
# =========================
# paper backbone + additionalregression(ranking->count), keeplabelreadlogicnotchange
# =========================
class Rank2CountRegressor(nn.Module):
    """takenetworkoutput (0~1)rankingvaluemapforcountvalue, used for η*L1Loss. 
    setcountbecomeleast squarescalibration learnableform: count = ReLU(a * rank + b)
    - global: scalar a,b
    - local : everygridindependent  a_i,b_i (16dimension)
    """
    def __init__(self, clipnum: int = 16):
        super().__init__()
        self.clipnum = clipnum
        # initialize fornearsimilarconstantetc.map(trainingwillauto-learntofitscale)
        self.a_g = nn.Parameter(torch.tensor(1.0))
        self.b_g = nn.Parameter(torch.tensor(0.0))
        self.a_l = nn.Parameter(torch.ones(clipnum))
        self.b_l = nn.Parameter(torch.zeros(clipnum))

    def forward(self, rank_global: torch.Tensor, rank_local: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        # rank_global: [B,1] or [B]
        # rank_local : [B,clipnum]
        if rank_global.dim() == 1:
            rg = rank_global.unsqueeze(1)
        else:
            rg = rank_global
        if rank_local.dim() == 1:
            rl = rank_local.unsqueeze(0)
        else:
            rl = rank_local

        pred_g = torch.relu(rg * self.a_g + self.b_g)              # [B,1]
        pred_l = torch.relu(rl * self.a_l.unsqueeze(0) + self.b_l.unsqueeze(0))  # [B,clipnum]
        return pred_g.squeeze(1), pred_l

class HSRankALTGVT(nn.Module):
    """usepaperin  ALTGVT-Small backbone(containoriginal Regression head), andadditionaladdonlearnableregression mapping. """
    def __init__(self, clipnum: int = 16, pretrained_backbone: bool = False):
        ...
        self.r2c = Rank2CountRegressor(clipnum=clipnum)

    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        # backbone Output: (rank_local[ B,16 ], rank_global[ B,1 ])
        rank_local, rank_global = self.backbone(x)
        # additionalregressionoutputcount
        pred_global_cnt, pred_local_cnt = self.r2c(rank_global.squeeze(-1), rank_local)
        return rank_local, rank_global, pred_local_cnt, pred_global_cnt
...
def train_epoch(
    ...
    for image, target_global, target_local, _ in loop:
        ...
        #   pred_local_cnt: [B,16] pred_global_cnt: [B]
        rank_local, rank_global, pred_local_cnt, pred_global_cnt = model(image)

        # (1) Ranking loss: when batch insideglobal countsame, willcause denom=0; at this pointonlytrainingregressionitem
        ...
        # (3) additionalregression(paper η*L1Loss): to global + local alldo L1
        lr_g = l1(pred_global_cnt.squeeze(), target_global)
        lr_l = l1(pred_local_cnt, target_local)
        lr = lr_g + lr_l

        loss = lg + float(lambda_local) * ll + float(eta_reg) * lr
...
def val_epoch(model: nn.Module, loader: DataLoader, device: str) -> Tuple[float, float]:
    """returnbased on"regressioncount"  (MAE, RMSE). meanwhileprintbased on"ranking output+linear calibration" tothanmetric. """
    model.eval()

    # --- 1) based onregression head  directcount ---
    preds_reg = []
    gts = []

    # --- 2) based onranking output linear calibration(retainyouoriginalfor evaluationlogic) ---
    preds_rank = []
    ...
    for image, target_global, _, _ in tqdm(loader, desc="val", ncols=110):
        ...
        _, rank_global, _, pred_global_cnt = model(image)

        preds_reg.append(float(pred_global_cnt.item()))
    ...
    # (A) regression MAE / RMSE
    mae_reg = sum(abs(p - gt) for p, gt in zip(preds_reg, gts)) / n
    rmse_reg = math.sqrt(sum((p - gt) ** 2 for p, gt in zip(preds_reg, gts)) / n)

    # (B) ranking output -> count  linearityregressioncalibration(originallogic)
    ...
    for a, gt in zip(preds_rank, gts):
        ...
        rmse_rank += (p - gt) ** 2
    mae_rank /= n
    rmse_rank = math.sqrt(rmse_rank / n)

    print(f"Val (REG)  MAE: {mae_reg:.2f}, RMSE: {rmse_reg:.2f} |  Val (RANK+LS) MAE: {mae_rank:.2f}, RMSE: {rmse_rank:.2f}")
    return mae_reg, rmse_reg
...
def main():
    ...
    # according topaper/hs-rank code: output layer(withandnewly addedregression mapping)usemorelarge learning rate(≈1000x)
    ...
    # newly added rank->count mapparameteralsoputto fast lr
    fast_params += list(model.r2c.parameters())
    ...
    for epoch in range(1, epochs + 1):
        train_epoch(epoch, model, optimizer, train_loader, loss_fn, device, lambda_local=float(CONFIG.get("lambda_local", 0.5)), eta_reg=float(CONFIG.get("eta_reg", 1.0)))
