...
- regression moduleInput:onlyuseglobal branchoutput rank_global(1dimension). 
...
CONFIG = {
    ...
    "rank_ckpt_path": "mymodel/mae6.04_rmse9.36_epoch112_RSOC-Building.pth",                 # for example "my_model/mae12.34_rmse20.11_epoch50_RSOC-Building.pth"
    "rank_ckpt_dir": "my_model",          # directoryinsideselectlatest .pth(according tofilemodifywheninterval)
    "rank_ckpt_glob": "*.pth",            # directorymatchrule
    ...
    # input 1dimension(only global), output 1dimension count
...
class RankToCountRegressor(nn.Module):
    """input 1dimension(rank_global), output 1dimension count"""
    def __init__(self, in_dim: int = 1, hidden: Optional[List[int]] = None, dropout: float = 0.0):
        super().__init__()
        ...
        # outputnotdo ReLU/Sigmoid: count canbylinear calibrationtoarbitrarypositive number; trainingwhenby loss pullbackgo
...
class RankFrozenPlusReg(nn.Module):
    """completestructure: ranking model(freeze) + regression head(cantraining)"""
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # onlyuse global branch  1dimensionoutputdoregression
        pred_count = self.regressor(rank_global)  # [B,1]
...
def main():
    ...
    reg = RankToCountRegressor(in_dim=1, hidden=list(CONFIG.get("reg_hidden", [128, 64, 16])), dropout=float(CONFIG.get("reg_dropout", 0.0))).to(device)
    unfreeze_module(reg)
        ...
        if epoch >= int(CONFIG["val_start"]):
            ...
            if mae < min_mae:
                ...
                out_path = os.path.join(
                    CONFIG["save_dir"], f"reg1d_mae{mae:.2f}_rmse{rmse:.2f}_epoch{epoch}_{dataset_label}.pth"
                )
