# mainprogram-dual-branchdensity map.py
# RSOC-Building ONLY
# inputafterstandmomentpartcross: 
...
# itsin a_1/a_2 formanualweight(in CONFIG insetting)
# ------------------------------------------------------------
...
CONFIG = {
    ...
    "a_1": 1.0,  # ranking branchloss weight
    "a_2": 1,  # density mapbranchloss weight(enabledynamicweightafterwillbyignore)
    "use_dynamic_weighting": True,  # useuncertainty(uncertainty)dynamicaddweighttwobranchloss
    "dyn_init_log_vars": (0.0, 0.0),  # (log_var_rank, log_var_density) initialvalue
...
class RSOCDualDataset(Dataset):
    def __init__(self, root: str, split: str = "train"):
        ...
        assert dataset_name == "RSOC-Building", "currentscriptonlyneedleto RSOC-Building"
...
# -------------------------
# letmodelselflearningtwobranchloss weight, andnotismanual a_1/a_2. 
# oftenusepractice: introduceentertwolearnable  log_var(correspondtasknoise/uncertainty), totalloss: 
#   L = exp(-s1)*L1 + exp(-s2)*L2 + (s1 + s2)
# itsin s1=log_var1, s2=log_var2. trainingwhenmeanwhileupdatenetworkparameterand s1/s2. 
# =========================
...
class DualBranchModel(nn.Module):
    def __init__(self, pretrained_altgvt: bool = False, use_dynamic_weighting: bool = True):
        ...
        # learnable dynamicweight(uncertaintyaddweight)
...
def train_one_epoch(epoch: int, model: DualBranchModel, loader: DataLoader,
    ...
    for image, target_local, target_global, gt_density, _ in pbar:
        ...
        # -------- total loss --------
        # L_rank = (Lg + 0.5*Ll)
        # L_total adoptdynamicweight: L = exp(-s1)*L_rank + exp(-s2)*L_density + (s1+s2)
...
def eval_rank_branch_metrics(model, loader, device, rank_loss_fn):
    """
    use"ranking branch"intest setonevaluationtwoclassmetric: 
      - rloss: average (Lg + 0.5*Ll), used forobserveranking lossitself convergesituation(exceedsmallexceedgood)
      - mae/rmse: reuse"purerankingprogram" practice: to rank_global do onceleast squareslinear calibration
        count_hat = k * rank_global + b, thenintest setoncompute MAE / RMSE(exceedsmallexceedgood)
    """
    model.eval()
    ...
    # 2) rank_global -> count  linearityregressioncalibrationneed cache
    preds_rank = []
    gts = []
    ...
    for image, target_local, target_global, _, _ in tqdm(loader, desc="eval_rank(test)", ncols=110):
        ...
        # --- ranking loss(andtrainingconsistent)---
        ...
        # --- collectused for MAE/RMSE   rank_global & gt_count ---
    ...
    # rank_global -> count  least squareslinearityfitting(closed-form solution, restoremomentpurerankingscript)
...
def main():
    ...
    # HSRankALTGVT internalwillautomatictryload alt_gvt_small.pth
    ...
    for epoch in range(1, int(CONFIG["epochs"]) + 1):
        ...
        # outputcurrentweight(every epoch)
        if getattr(model, "loss_balancer", None) is not None:
