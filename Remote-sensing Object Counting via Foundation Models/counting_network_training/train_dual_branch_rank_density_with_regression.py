...
evaluationscript: 
- read"dual-branch(ranking+density map)"model checkpoint(state_dict)
- useclosed-form solutionleast squares, takeranking branch rank_global map to count: y = a*x + b
- fusecombine: final = w_rank * y_rank + w_density * y_density
- outputranking/density/fusecombinethreesetresult  MAE, RMSE

default: use train setfitting (a,b), use test setevaluation. 
ifwantin test onfittingagainevaluation(moreoptimistic), take FIT_SPLIT changefor "test". 

Note:thisscriptwilldynamicloadyouprevious training script, withreusecompleteallconsistent : 
- DualBranchModel
- RSOCDualDataset
- CONFIG(withand density_scale etc.)

youneedensure TRAIN_SCRIPT pointtoyoutraining scriptfilename(usuallyandthisscriptputsamedirectoryi.e.can). 
"""
...
# =========================
EVAL_CONFIG = {
    # training script(Iprevioustoyou thatportion: manualWeights version)
    "train_script": "mainprogram-dual-branchdensity map.py",

    # traininggoodsave  .pth(state_dict)path
    # For example:"my_model_dual_branch/best_dual_branch_....pth"
    "checkpoint": "my_model_dual_branch/best_dual_branch_DMAE12.80_DRMSE19.26_RLOSS0.284_RMAE8.682_RRMSE12.518.pth",

    # datasetroot directory(andtraining scriptconsistent)
    "dataset_root": "dataset",

    # linearityfittingusewhich split: "train" or "test"
    "fit_split": "train",

    # batch_size(evaluationcanopenlargeonepoint, GPU memorynotenoughthentunesmall)
    "batch_size": 8,

    # Windows belowforceintensesuggest num_workers=0(avoid DataLoader multiprocessing pickling reportwrong)
    "num_workers": 0,

    # fusecombineweight: final = w_rank * rank_calib + w_density * density_count
    "w_rank": 0.5,
    "w_density": 0.5,
...
def load_training_module(train_script_path: str):
    """from .py filedynamicloadmodule, avoidrestoremakeonelargeheapmodel/datasetcode. """
    if not os.path.isfile(train_script_path):
        raise FileNotFoundError(f"TRAIN_SCRIPT not found: {train_script_path}")

    # useonefixed , canreuse modulename; meanwhiletakeitregisterenter sys.modules, 
    # thissampleeven ifyoumanualtake num_workers tunelarge, alsomoreeasyinchildprocessinjustconfirm import. 
    ...
    # registerto sys.modules, raiseraise Windows multiprocessingcompatibility
    import sys
...
def fit_linear_closed_form(x: torch.Tensor, y: torch.Tensor) -> Tuple[float, float]:
    """closed-form solutionleast squares: y ≈ a*x + b
    return (a, b). 
    ...
    if float(vx.item()) < 1e-12:
        # extremeendsituation: x basethisisoftennumber, notmethodfittingslope
        a = 0.0
        b = float(my.item())
        return a, b
...
def main():
    ...
    if hasattr(train_mod, "CONFIG") and isinstance(train_mod.CONFIG, dict):
        ...
        # label_dir usuallyisbased on dataset_root  ; ifyoutraining scriptinsidewritedead , alsosynchronizechangeonebelow
        if "label_dir" in train_mod.CONFIG:
            # if label_dir thiscomeismutualtopath, thenaccording to dataset_root pushleadto dataset/local pseudo
            # youalsocanmanualchange toyouself absolutelytopath
...
if __name__ == "__main__":
    # Windows + DataLoader multiprocessingwhenneed; even if num_workers=0 alsonoharm
