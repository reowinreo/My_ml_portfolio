...
# ===== Backbone (VGG16) =====
# use VGG16   features as backbone, truncatein pool3/pool4/pool5, getthreescalefeature: 
#   pool3 -> 64x64 (C=256)  -> 1x1 conv -> x1: 128x64x64
#   pool4 -> 32x32 (C=512)  -> 1x1 conv -> x2: 256x32x32
#   pool5 -> 16x16 (C=512)  -> (optional 1x1) -> x3: 512x16x16
#
# thentake (x1,x2,x3) originalsealnotmovefeedyouoriginalcome  Regression head(keep"dual-branch"structurenotchange). 
#
# pretrained weights: 
# - if CONFIG["pretrained_backbone"]=True and vgg16_ckpt forempty, willuse torchvision insideset  ImageNet weight(canautomaticbelowload/fromcacheread). 
# - ifyouwantawaylineuse, pleasetakebelowloadgood weight filepathfillto CONFIG["vgg16_ckpt"](for example vgg16-397923af.pth). 
...
CONFIG = {
    ...
    "lambda_local": 0.5,          # λ
    "pretrained_backbone": True,  # True=use VGG16 ImageNet pretrained weights(orfrom vgg16_ckpt load)
    "vgg16_ckpt": "vgg16-397923af.pth",            # optional: awaylineweight filepath, for example "vgg16-397923af.pth"
...
# =========================
# VGG16 backbone(output rank_local / rank_global), keeplabelreadlogicnotchange
# =========================
def _strip_prefix(state_dict: dict, prefix: str) -> dict:
    return {(k[len(prefix):] if k.startswith(prefix) else k): v for k, v in state_dict.items()}


def _load_vgg16_features(pretrained: bool = True, ckpt_path: str = "") -> nn.Sequential:
    """return torchvision   vgg16.features(notcontain classifier). 

    - pretrained=True and ckpt_path forempty: use torchvision insideset ImageNet weight(maybeautomaticbelowload). 
    - ckpt_path non-empty: fromlocal .pth load(awaylinecanuse). 
    """
    try:
        from torchvision.models import vgg16
        # newversion torchvision has WeightsEnum
        try:
            from torchvision.models import VGG16_Weights  # type: ignore
            weights = VGG16_Weights.IMAGENET1K_V1 if pretrained and not ckpt_path else None
            vgg = vgg16(weights=weights)
        except Exception:
            # oldversion torchvision
            vgg = vgg16(pretrained=bool(pretrained and not ckpt_path))
    except Exception as e:
        raise ImportError(
            "import torchvision fail. pleasefirstinstallmatchyou torch version  torchvision(elsenomethoduse VGG16 backbone). "
            f" originalError: {e}"
        )
...
class VGG16Backbone(nn.Module):
    """VGG16 backbone(according to yourthistimes newwantmethod): 

    - retainto pool3(input 512x512 whenget 64x64  feature)
    - **skip pool4 / pool5**, let block4 + block5 allin 64x64 oncompute
    - therefore block5 endendoutputkeep **64x64x512**
    - afteruseconvolution head: 3x3(512→256) + 3x3(256→64) + 1x1(64→1), get y=[B,1,64,64]
    - y againenterenteryouoriginalcome "local/globaldual-branch"part(reuse Regression inside _block/c16/output, notchangedual-branchstructure)
    """

    def __init__(self, clipnum: int = 16, pretrained_backbone: bool = True, vgg16_ckpt: str = ""):
        super().__init__()
        self.features = _load_vgg16_features(pretrained=pretrained_backbone, ckpt_path=vgg16_ckpt)

        # you proposed  y generate head: 64x64x512 -> 64x64x1
        self.y_head = nn.Sequential(
            ...
            nn.ReLU(),  # andoriginal Regression.res classsimilar: let y non-negative
        )

        # notchangedual-branch: reuseyouoriginal Regression   _block/c16/output
        #(Note:Regression inside v1/v2/v3/res inherenotwillbyuseto)
        self.regression = Regression(clipnum=clipnum)

    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        pool_cnt = 0

        # run VGG16 features, butskipNo. 4/5 timespooling(pool4/pool5)
        for layer in self.features:
            if isinstance(layer, nn.MaxPool2d):
                pool_cnt += 1
                if pool_cnt >= 4:
                    # skip pool4 / pool5: notdownsample
                    continue
            x = layer(x)

        # logicdiscusson(input 512x512)at this point x = [B,512,64,64]
        y = self.y_head(x)  # -> [B,1,64,64]

        # ===== notchangedual-branch: y -> (rank_local[16], rank_global[1]) =====
        y_c16 = self.regression._block(y)
        y_flat = y.view(y.size(0), -1)
        y_concat = torch.cat([y_flat, y_c16], dim=1)
        y_global = self.regression.output(y_concat)
        return y_c16, y_global


class HSRankVGG16(nn.Module):
    """use VGG16 backbone(output rank_local / rank_global). """
    def __init__(self, clipnum: int = 16, pretrained_backbone: bool = True, vgg16_ckpt: str = ""):
        super().__init__()
        self.backbone = VGG16Backbone(
            clipnum=clipnum,
            pretrained_backbone=pretrained_backbone,
            vgg16_ckpt=vgg16_ckpt,
        )
...
def main():
    ...
    vgg16_ckpt = str(CONFIG.get("vgg16_ckpt", ""))
    model = HSRankVGG16(clipnum=int(CONFIG["clipnum"]), pretrained_backbone=pretrained_backbone, vgg16_ckpt=vgg16_ckpt).to(device)
    if CONFIG["resume"]:
