...
class Regression(nn.Module):
    ...
    def block(self, y, clipnum=clipnum):
        n = int(math.sqrt(clipnum))
        b, h, w = y.size(0), y.size(-2) // n, y.size(-1) // n
        y_c16 = torch.zeros(y.size(0), clipnum).to(device)
        num = 0
    ...
    def forward(self, x1, x2, x3):
        ...
        y_concat = self.output(y_concat)
        return y_c16, y_concat

        # y = y.view(y.size(0), -1)
        # y_concat = self.output(y)
        # return 0, y_concat
...
class SBlock(Block):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        super(SBlock, self).__init__(
            ...
            act_layer=act_layer, norm_layer=norm_layer, sr_ratio=sr_ratio
        )
...
def alt_gvt_small(pretrained=False, **kwargs):
    ...
    if pretrained:
        '''download from https://github.com/Meituan-AutoML/Twins/alt_gvt_small.pth'''
        checkpoint = torch.load('alt_gvt_small.pth')  # todo pass path as argument
...
def alt_gvt_base(pretrained=False, **kwargs):
    ...
    model.default_cfg = _cfg()
    if pretrained:
        '''download from https://github.com/Meituan-AutoML/Twins/alt_gvt_base.pth'''
        checkpoint = torch.load('alt_gvt_base.pth')  # todo pass path as argument
        model.load_state_dict(checkpoint, strict=False)
...
def alt_gvt_large(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        checkpoint = torch.load('alt_gvt_large.pth')  # todo pass path as argument
        model.load_state_dict(checkpoint, strict=False)
        # print("load transformer pretrained")
    return model
...
#     model = alt_gvt_small(pretrained=True)
