...
def select_exemplar_boxes(dataset_label: str, split_for_exemplar: str, num_images: int):
    ...
    Note:here  rel_xywh ismutualtowhole image normalizationcoordinate; aftercontinuewillmap toarbitraryimage. 
    """
    ...
    for img_path in img_list:
        ...
        rel_xywh = _abs_xywh_to_rel_xywh(roi, w, h)
        picked.append({"rel_xywh": rel_xywh, "label": bool(label), "src_image": base})
        print(f"[Exemplar] alreadyrecord rel_xywh={tuple(round(v,4) for v in rel_xywh)} | label={'pos' if label else 'neg'}")
