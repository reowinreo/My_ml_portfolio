...
def draw_boxes(img_path, roi_xyxy, gd_boxes, sam_boxes_pass1, sam_boxes_pass2, final_boxes, img_id, text):
    try:
...
def main():
    ...
    for s in chosen:
        ...
        # ---------- 2) SAM3 pass1: insohas GD boxinside(ROI inside) ----------
        sam_boxes_pass1_roi = []
        ...
        print(f"  [SAM3 pass1@ROI] in-box merged = {len(sam_boxes_pass1_roi)}")

        # ---------- 3) SAM3 pass2: ROI allimage(plain text, notuseanyprompt box) ----------
        sam_boxes_pass2_roi = []
        ...
        print(f"  [SAM3 pass2@ROI] roi-text-only merged = {len(sam_boxes_pass2_roi)}")
        ...
        print(f"  [Count@ROI] pseudoROI={final_count:.2f} | trueROI={s.get('true_count', 0.0):.2f}")

        # visualize: take ROI coordinate system boxtranslatebackoriginalimagecoordinate system
        ...
        draw_boxes(img_path, roi_xyxy, gd_boxes_full, sam1_full, sam2_full, final_full, img_id, text)

    # ---------- 5) pseudo labelleaderboard ----------
    ranking_pseudo = sorted(chosen, key=lambda x: x.get("pseudo_count", 0.0), reverse=True)
    print("\n[Ranking] combineandcount pseudo labelleaderboard(ROI insidecount): ")
    for rank, s in enumerate(ranking_pseudo, start=1):
        roi = s.get("roi_xyxy", None)
        roi_str = f"ROI=({roi[0]},{roi[1]},{roi[2]},{roi[3]})" if roi else "ROI=None"
        print(f"  Rank {rank:2d}: ID={s['id']:2d} | PseudoROI={s.get('pseudo_count', 0.0):.2f} | TrueROI={s.get('true_count', 0.0):.2f} | {roi_str}")
