...
SAM3_PROMPT_MAP = {
    "RSOC-Building": "building.structure",
...
# Description:
# - willoriginalfirst  DeepDataSpace (DDS) API callreplaceforlocal GroundingDINO inference. 
# - restpipeline(SAM3 twiceinference, deduplicationcombineand, leaderboard, visualize)keepnotchange. 
#
...
# - GD_BOX_THRESHOLD: correspond"detectionboxconfidencethreshold"
# - GD_TEXT_THRESHOLD: correspond"textmatchthreshold"(onegeneralomitlowin box threshold)
#
# Note:transformers differentversion  post_process_grounded_object_detection APIomithasdifference, 
#       heredo compatiblehandle(referenceyou  task2 script). 

GDINO_MODEL_ID = os.getenv("GDINO_MODEL_ID", "grounding_dino_base")  # orone whochange toyou localmodeldirectory
GD_BOX_THRESHOLD = float(os.getenv("GD_BOX_THRESHOLD", str(GD_BBOX_THRESHOLD)))
GD_TEXT_THRESHOLD = float(os.getenv("GD_TEXT_THRESHOLD", "0.15"))
...
def call_dinox_api(img_path, prompt_text):
    """
    [compatibleoriginalscriptAPI]return boxes: list[[x1,y1,x2,y2], ...] (pixel coordinates, xyxy)

    originalscriptuse is DDS API, herechangeforlocal GroundingDINO inference, 
    withguarantee main() inotherlogiccompleteallnotneedchange. 
    """
    ...
    # inputs is BatchEncoding, include input_ids/pixel_values etc.
    ...
    try:
        ...
        # oldversion: secondparameterpropagate input_ids; andmanualdothresholdfilter
        ...
        keep = scores > GD_BOX_THRESHOLD
        scores = scores[keep]
        boxes = boxes[keep]
        processed = {"scores": scores, "boxes": boxes}
    ...
    # directreturnsohasfullenoughthreshold box(noneedclassmap)
