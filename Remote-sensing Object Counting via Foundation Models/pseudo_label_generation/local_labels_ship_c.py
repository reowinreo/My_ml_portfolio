...
# ---------- visualize ----------
OUT_DIR = os.path.join(ROOT_DATA_DIR, "viz_ship_method_C")
...
PERTURB_ENABLE = True
PERTURB_DELETE_RATIO = 0.4
PERTURB_MOVE_RATIO   = 0.1
MOVE_SHIFT_MIN = 10
MOVE_SHIFT_MAX = 50
...
def sam3_text_plus_boxes_masks(processor: Sam3Processor, pil_img: Image.Image, boxes_xywh_list, labels_list, prompt_text: str):
    ...
    fixpoint(key): 
    - **notneed**in add_geometric_prompt(...) afteragaincall set_text_prompt(...) andtakeitsoutputwhenfinalresult, elsewilltakegeometric prompt inferenceresultoverwritedrop, 
      cause"add 3 /10  ROI andplain textcompleteallsame" degeneratenowimage. 
    - hereadopt"B method" mainPipeline:first set_image, again set_text_prompt lettextconceptwrite state, thenone by one add_geometric_prompt(thisfunctionitselfusuallywilltriggerinference), 
      finaldirectfromfinallyoncegeometryinference  state intake masks/boxes/scores(ifnogeometryboxthendegenerateforplain text). 

    remark: different sam3 versionreturn valuemaybe output dict or state dict; thisfunctionas much as possiblecompatible. 
    """
    ...
    # nogeometrybox: degeneratebecomeplain text
    if len(boxes_xywh_list) == 0:
        return sam3_text_only_masks(processor, pil_img, prompt_text)

    width, height = pil_img.size
    state = processor.set_image(pil_img)

    # ifimplementsupport reset, firstclearonebelow(new state onegeneralnoneed, butmorestable)
    try:
        processor.reset_all_prompts(state)
    except Exception:
        pass

    # firstwritetextconcept(notneeduseit  output overwrite state; weneedretainsameone state continueaddgeometric prompt)
    try:
        _ = processor.set_text_prompt(state=state, prompt=prompt_text)
    except Exception:
        # someversionmaybenotneed/notsupporttext, hereignorei.e.can
        pass
    ...
    for (xywh, lab) in zip(boxes_xywh_list, labels_list):
        ...
        # add_geometric_prompt usuallywilltriggeronceinference, andtakeresultwriteenter state
        state_use = processor.add_geometric_prompt(box=norm_box_cxcywh, label=bool(lab), state=state_use)
        last = state_use

    od = _sam3_state_to_output_dict(last if last is not None else state_use)
    masks = _to_numpy(od["masks"])
    boxes  = _to_numpy(od["boxes"])
    scores = _to_numpy(od["scores"])
    return masks, boxes, scores
...
def pick_exemplar_crops(image_paths, num_ex=10, seed=12):
...
def _rand_signed_step(minv: int, maxv: int) -> int:
...
def translate_mask_bool(mask_bool: np.ndarray, dx: int, dy: int):
    ...
    # objectregion(dst)andsourceregion(src)slice
    # dst_x = [max(0, dx) : min(W, W+dx)]
    # src_x = [max(0, -dx): min(W, W-dx)]
    dst_x1 = max(0, dx)
    dst_y1 = max(0, dy)
    dst_x2 = min(W, W + dx)
    dst_y2 = min(H, H + dy)
...
def perturb_masks_list(masks_list, delete_ratio=0.0, move_ratio=0.0, shift_min=10, shift_max=50):
    ...
    if move_k > 0:
        ...
        random.shuffle(idxs)
        move_set = set(idxs[:move_k])
        new_masks = []
        for i, m in enumerate(masks):
            if i in move_set:
                dx = _rand_signed_step(shift_min, shift_max)
                dy = _rand_signed_step(shift_min, shift_max)
...
def main():
    ...
    exemplar_crops = pick_exemplar_crops(ex_imgs, num_ex=3, seed=12)
    if len(exemplar_crops) == 0:
        print("[Warn] noselecttoany exemplar ROI. aftercontinuewilldegeneratefornotusecontent hint. ")
