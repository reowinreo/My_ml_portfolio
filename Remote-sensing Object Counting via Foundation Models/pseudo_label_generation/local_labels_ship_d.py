...
# ---------- visualize ----------
OUT_DIR = os.path.join(ROOT_DATA_DIR, "viz_ship_method_D")
...
PERTURB_ENABLE = True
PERTURB_DELETE_RATIO = 0.3
PERTURB_MOVE_RATIO   = 0
MOVE_SHIFT_MIN = 10
MOVE_SHIFT_MAX = 50
...
def main():
    ...
    exemplar_crops = pick_exemplar_crops(ex_imgs, num_ex=10, seed=123)
    ...
    viz = overlay_masks_and_boxes(out["bgr"], out["masks"], idx_text="")

    os.makedirs(OUT_DIR, exist_ok=True)
    save_path = os.path.join(OUT_DIR, f"{stem}_D.jpg")
    cv2.imwrite(save_path, viz)
    print(f"[Saved] {save_path}")

    if SHOW_WINDOW:
        cv2.imshow("D) GD+SAM3 + 10 exemplars", viz)
