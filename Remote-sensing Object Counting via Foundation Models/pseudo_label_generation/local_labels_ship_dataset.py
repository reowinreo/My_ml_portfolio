...
TARGET_IMAGE_NAME = "P0340.png"   # youwantrun thatimagename(withextension)
...
# ---------- visualize ----------
OUT_DIR = os.path.join(ROOT_DATA_DIR, "viz_ship_four_methods")
...
def sam3_text_plus_boxes_masks(processor: Sam3Processor, pil_img: Image.Image, boxes_xywh_list, labels_list, prompt_text: str):
    ...
    youencounterto nowimage(3ROI and 10ROI severalaboutsame)confirmrealverymaybefrom"againtimes set_text_prompt() clearempty/overwritegeometric prompt state" problem. 
    thereforeherechange to: 
      1) set_image
      2)(optional)reset_all_prompts
      3) set_text_prompt(onlycallonce, used forbuildstandtextconcept)
      4) add_geometric_prompt(addboxprompt)
      5) directfrom state take masks(if empty, againtryuse processor  otherinferenceAPI; finallyonly thendegeneratefor set_text_prompt fallback)

    thissamplecanmaximumprocessdegreeguaranteegeometric promptnotwillbyaftercontinuetextcalloverwrite. 
    """
    ...
    # 1) newimage -> new state(avoidcross tile / crossmethodresidual)
    state = processor.set_image(pil_img)

    # 2) clearemptyoldprompt(ifAPIsavein)
    try:
        processor.reset_all_prompts(state)
    except Exception:
        pass

    # 3) onlycallonce set_text_prompt(buildstandtextconcept/semantic)
    state_text = None
    try:
        state_text = processor.set_text_prompt(state=state, prompt=prompt_text)
    except Exception:
        state_text = None

    # has implementreturn dict/output, has return state; hereunifyuse"moreimage state  that"
    state_use = state_text if state_text is not None else state

    # 4) addgeometric prompt
    for (xywh, lab) in zip(boxes_xywh_list, labels_list):
        ...
        box_xywh = torch.tensor([[float(x), float(y), float(w), float(h)]], dtype=torch.float32, device=DEVICE)
        ...
        try:
            ...
            # someimplement add_geometric_prompt notconsume dict state, fall backuseoriginal state
            try:
                state_use = processor.add_geometric_prompt(box=norm_box_cxcywh, label=bool(lab), state=state)
            except Exception:
                pass

    # 5) directfrom state extractoutput(prefer)
    od = _sam3_state_to_output_dict(state_use)
    masks = _to_numpy(od["masks"])
    boxes  = _to_numpy(od["boxes"])
    scores = _to_numpy(od["scores"])

    # 6) if masks stillforempty: tryotherinferenceAPI(differentversionmaybehas)
    def _empty_masks(m):
        if m is None:
            return True
        try:
    ...
    if _empty_masks(masks):
        out2 = None
        # try predict / infer / run_inference etc.(ifsavein)
        for fn in ("predict", "infer", "run_inference", "forward"):
            if hasattr(processor, fn):
                try:
                    out2 = getattr(processor, fn)(state=state_use)
                    break
                except TypeError:
                    try:
                        out2 = getattr(processor, fn)(state_use)
                        break
                    except Exception:
                        out2 = None
                except Exception:
                    out2 = None
        if out2 is not None:
            od2 = _sam3_state_to_output_dict(out2)
            masks = _to_numpy(od2["masks"])
            boxes  = _to_numpy(od2["boxes"])
            scores = _to_numpy(od2["scores"])

    # 7) finallyfallback: againcallonce set_text_prompt(butthisonestepmaybewilloverwritegeometric prompt, soonlyinmustneedwhendo)
    if _empty_masks(masks):
        try:
            out3 = processor.set_text_prompt(state=state_use, prompt=prompt_text)
            od3 = _sam3_state_to_output_dict(out3)
            masks = _to_numpy(od3["masks"])
            boxes  = _to_numpy(od3["boxes"])
            scores = _to_numpy(od3["scores"])
        except Exception:
            pass
...
def overlay_boxes_only(img_bgr, boxes_xyxy, title_text=""):
    ...
    if title_text:
        cv2.putText(out, title_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.85, (255, 255, 255), 2, cv2.LINE_AA)
    return out
...
def main():
    ...
    # ===================== fixedspecifyneedrun objectimage(onlychangeherethisonerowi.e.can) =====================
    TARGET_IMAGE_NAME = "P0703.png"  # TODO: change toyouwantrun thatimagefilename(mustwithextension, if .png/.jpg)
    # =====================================================================================

    # 1) listcanuseimage(ex_imgs used for"ROI exampleprompt"extracttake; sample_imgs used forfindtoobjectimage)
    ...
    # 2) fixedselect 1 imageevaluationimage(fourkindmethodtotalusesameoneimage)
    sample_imgs = list(sample_imgs)
    target_lower = TARGET_IMAGE_NAME.strip().lower()
    if not target_lower:
        raise RuntimeError("TARGET_IMAGE_NAME forempty. pleasein main() insidetake TARGET_IMAGE_NAME change toyouneedrun imagename(if P0053.png). ")
    ...
    stem = os.path.splitext(base)[0]
    print(f"\n[Pick] evaluationimage(fixedspecify): {base}")

    # 3) load SAM3 processor(oncei.e.can)
    processor = build_sam3_processor()

    # 4) runmethod A / B(firstnotuse ROI exampleprompt)
    print("\n[Run] A) GD only ...")
    out_gd = run_gd_only(img_path)

    print("\n[Run] B) GD + SAM3 (text only) ...")
    out_b = run_one_image(processor, img_path, exemplar_crops=[])

    # 5) visualize + save(onlysave image)
    os.makedirs(OUT_DIR, exist_ok=True)

    def _show_and_save(name: str, viz_bgr):
        save_path = os.path.join(OUT_DIR, f"{stem}_{name}.jpg")
        cv2.imwrite(save_path, viz_bgr)
        print(f"  - saved: {save_path}")
        if SHOW_WINDOW:
            cv2.imshow(name, viz_bgr)
            ...
            if key == 27:  # ESC
                print("[User] ESC pressed, stop display.")
                return False
        return True

    # A viz
    if out_gd is not None:
        viz_a = overlay_boxes_only(out_gd["bgr"], out_gd["gd_boxes"], title_text=f"A) GD only | {base}")
        if not _show_and_save("A_gd_only", viz_a):
            return
    else:
        print("[Warn] GD-only readfail. ")

    # B viz
    def _viz_mask(out, title):
        if out is None:
            return None
        return overlay_masks_and_boxes(out["bgr"], out["masks"], idx_text=title, gt_cnt=None, pseudo_cnt=None)

    viz_b = _viz_mask(out_b, f"B) GD+SAM3(text) | {base}")
    if viz_b is not None:
        if not _show_and_save("B_gd_sam3_text", viz_b):
            return
    else:
        print("[Warn] B) outputforempty. ")

    # 6) method C: letyoumanualbox 3  ROI(exampleprompt), againrunonce
    print("\n[ROI] nowinpleaseyoumanualboxselect 3  ROI(used for C method: GD+SAM3+3ROI). ")
    exemplar_crops_3 = pick_exemplar_crops(ex_imgs, num_ex=3, seed=123)
    if len(exemplar_crops_3) == 0:
        print("[Warn] younoselectto ROI, method C willdegeneratefor text-only(andmethod B classsimilar). ")

    print("\n[Run] C) GD + SAM3 + 3 ROI exemplars ...")
    out_c = run_one_image(processor, img_path, exemplar_crops=exemplar_crops_3)

    viz_c = _viz_mask(out_c, f"C) GD+SAM3 + 3 exemplars | {base}")
    if viz_c is not None:
        if not _show_and_save("C_gd_sam3_ex3", viz_c):
            return
    else:
        print("[Warn] C) outputforempty. ")

    # 7) method D: letyoumanualbox 10  ROI(exampleprompt), againrunonce
    print("\n[ROI] nowinpleaseyoumanualboxselect 10  ROI(used for D method: GD+SAM3+10ROI). ")
    exemplar_crops_10 = pick_exemplar_crops(ex_imgs, num_ex=10, seed=456)
    if len(exemplar_crops_10) == 0:
        print("[Warn] younoselectto ROI, method D willdegeneratefor text-only(andmethod B classsimilar). ")

    print("\n[Run] D) GD + SAM3 + 10 ROI exemplars ...")
    out_d = run_one_image(processor, img_path, exemplar_crops=exemplar_crops_10)

    viz_d = _viz_mask(out_d, f"D) GD+SAM3 + 10 exemplars | {base}")
    if viz_d is not None:
        if not _show_and_save("D_gd_sam3_ex10", viz_d):
            return
    else:
        print("[Warn] D) outputforempty. ")
