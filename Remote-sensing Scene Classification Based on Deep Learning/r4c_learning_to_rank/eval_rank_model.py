# -*- coding: utf-8 -*-
"""
Script 3: evaluate the ranking-counting model (based on SAM3 pseudo labels)
- Compute Spearman correlation between global scores and pseudo counts
- Compute pairwise ranking accuracy of the global ordering (how many pairs are ordered consistently)
"""
...
from train_rank_model import RankCountingNet  # Reuse the model definition directly
...
class PseudoCountingDataset(Dataset):
    def __init__(self, pseudo_path, split="test", transform=None):
        ...
        if len(self.samples) == 0 and split != "train":
            # If this split is empty, fall back to train for evaluation
            self.samples = [e for e in entries if e["split"] == "train"]
            print(f"[Dataset] split={split} is empty, falling back to train, {len(self.samples)} samples")
        else:
...
def spearman_correlation(x, y):
    """
    Simple Spearman rho implementation:
    - Rank x and y separately, then compute Pearson correlation
    """
    x = np.asarray(x)
    y = np.asarray(y)
    ...
    def rankdata(a):
        ...
        # Ties are not handled (usually negligible); use average ranks for strict handling
        return ranks
...
def pairwise_ranking_accuracy(counts, scores, max_pairs=100000):
    """
    Compute pairwise ranking accuracy of the global ordering:
    - Select all (i,j) pairs or a random subset
    - A pair is correct if sign(count_i - count_j) == sign(score_i - score_j)
    """
...
def eval_model(pseudo_path, model_path, img_size=512, split="test"):
    ...
    print(f"[Eval] Spearman correlation: {rho:.4f}")
    print(f"[Eval] Pairwise ranking accuracy: {acc:.4f}")
    print(f"[Eval] Number of evaluated samples: {len(all_counts)}")
