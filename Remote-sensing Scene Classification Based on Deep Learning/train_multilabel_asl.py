# -*- coding: utf-8 -*-
"""
Multi-label scene classification with a pretrained ResNet50 + Asymmetric Loss (ASL).

Same structure as the BCE baseline (train_multilabel_bce.py) and the Focal Loss variant
(train_multilabel_focal_loss.py); only the loss is replaced with Asymmetric Loss. Data,
model, input size, batch size, optimizer, epochs, learning-rate schedule, augmentation,
threshold, and metrics are all identical.

Asymmetric Loss (Ridnik et al., ICCV 2021):
    p   = sigmoid(logit)
    p_m = max(p - m, 0)
    L   = -y * (1-p)^gamma_pos * log(p) - (1-y) * p_m^gamma_neg * log(1-p_m)
Paper defaults (their Table 2 optimum): gamma_pos = 0, gamma_neg = 4, m = 0.05.
...
CONFIG = {
    ...
    "asl_gamma_pos": 0,      # gamma_pos: focusing on positives (0 = plain CE on positives)
    "asl_gamma_neg": 6,      # gamma_neg: focusing on negatives
    "asl_margin": 0.05,      # m: probability shift (hard threshold on negatives)
}
...
class AsymmetricLoss(nn.Module):
    """
    Per-label binary Asymmetric Loss (ASL) for multi-label classification, numerically
    stable via clamped logarithms.
    ...
    def forward(self, logits, targets):
        ...
        p_m = torch.clamp(p - self.margin, min=0.0)          # probability shift (negatives only)

        # Per-element cross-entropy, split into positive and negative terms.
        ce = targets * torch.log(p.clamp(min=self.eps)) \
             + (1 - targets) * torch.log((1 - p_m).clamp(min=self.eps))

        # Focusing factors, split into positive and negative terms.
        modulating = targets * (1 - p) ** self.gamma_pos \
                     + (1 - targets) * p_m ** self.gamma_neg
...
def main():
    ...
    criterion = AsymmetricLoss(gamma_pos=cfg["asl_gamma_pos"],
                               gamma_neg=cfg["asl_gamma_neg"],
                               margin=cfg["asl_margin"])
    ...
    report_and_save(best_state, "best_model_asl", best_val_f1, best_epoch)
    report_and_save(best_state_after300, "best_model_asl_after300", best_val_f1_after300, best_epoch_after300)
