# -*- coding: utf-8 -*-
"""
Multi-label scene classification with a pretrained ResNet50 + BCEWithLogitsLoss.

Dataset (AID-ML, 3000 images / 30 classes / 17 labels, images_tr:images_test = 8:2):
  - dataset/multilabel .csv : 3000 rows; column 0 is the image name (no .jpg), the
    remaining 17 columns are 0/1 multi-label targets.
  - dataset/images_tr      : training images (30 classes x 80 images = 2400).
  - dataset/images_test    : test images (30 classes x 20 images = 600).

Labels (17 classes):
  airplane, bare-soil, buildings, cars, chaparral, court, dock, field,
  grass, mobile-home, pavement, sand, sea, ship, tanks, trees, water

Training setup:
  - 512x512 inputs; pretrained ResNet50 with only the final fc layer replaced by a
    17-unit head.
  - Adam optimizer, batch_size=16.
  - 400 epochs with a piecewise learning-rate schedule:
    epochs 1-200 -> 1e-5, 201-300 -> 1e-6, 301-400 -> 5e-7.
  - The training set is expanded 5x with fixed geometric variants (identity,
    horizontal/vertical flip, 90-degree rotations).
"""
...
class MultiLabelDataset(Dataset):
    ...
    def __init__(self, samples, transform=None, expand=1):
        """
        samples: list of (absolute image path, 17-dim float label tensor).
        """
...
# ============================ Metrics (example-based) ============================
def example_based_metrics(preds, targets):
    """
    Example-based multi-label metrics: compute P/R/F1/F2 over the C labels of each
    sample, then average over samples. Samples with a zero denominator are scored 0
    (matching sklearn's zero_division=0). Returns (precision, recall, f1, f2).
    """
    preds = preds.float()
    ...
    f1 = torch.where(denom_f1 > 0, 2 * precision * recall / denom_f1,
                     torch.zeros_like(precision))
    denom_f2 = 4 * precision + recall
    f2 = torch.where(denom_f2 > 0, 5 * precision * recall / denom_f2,
                     torch.zeros_like(precision))
...
def label_based_macro_metrics(preds, targets):
    """
    Label-based macro-averaged precision/recall: accumulate TP/FP/FN per label over all
    samples, compute P/R for each of the C labels, then average (equal weight per label).
    Labels with a zero denominator are scored 0. Returns (precision, recall).
    """
    preds = preds.float()
    targets = targets.float()
    tp = (preds * targets).sum(dim=0)          # per-label TP, shape (C,)
...
def evaluate(model, loader, criterion, device):
    """Return (mean loss, metrics dict).

    The metrics dict contains 6 entries:
      ex_P / ex_R        example-based precision / recall
      label_P / label_R  label-based (macro) precision / recall
      ex_F1 / ex_F2      example-based F1 / F2
    """
    model.eval()
    total_loss = 0.0
...
def main():
    ...
    # Hold out 10% of the training set for validation (fixed seed).
    ...
    for epoch in range(1, cfg["epochs"] + 1):
        # Apply the piecewise learning rate.
